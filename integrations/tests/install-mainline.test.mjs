...
test('Mainline bootstrap pins CI candidate hashes and does not enable stable installation', () => {
  ...
  ]) assert.ok(text.includes(hash), hash);
...
for (const runtime of windowsRuntimes) {
  test(runtime + ': actual verification helper rejects altered bytes and releases file handles', () => {
    ...
    const script = `${parse}; $definition=$ast.Find({param($node) $node -is [Management.Automation.Language.FunctionDefinitionAst] -and $node.Name -eq 'Assert-MainlineAsset'},$true); Invoke-Expression $definition.Extent.Text;
      Assert-MainlineAsset ${quote(fixture)} 7 ${quote(hash)};
      $failed=$false; try { Assert-MainlineAsset ${quote(fixture)} 8 ${quote(hash)} } catch { $failed=$true }; if(-not $failed){throw 'Wrong size accepted'};
      $failed=$false; try { Assert-MainlineAsset ${quote(fixture)} 7 ('0'*64) } catch { $failed=$true }; if(-not $failed){throw 'Wrong digest accepted'};
  ...
  for (const mode of ['download-error', 'missing', 'wrong-size', 'wrong-hash']) {
    test(runtime + ': irm/iex failure ' + mode + ' cannot install and cleans only its temporary download', () => {
      ...
      const script = `$ErrorActionPreference='Continue'; $protocol=[Net.ServicePointManager]::SecurityProtocol; $userPath=[Environment]::GetEnvironmentVariable('Path','User'); $script:downloads=0;
        ...
        if(Test-Path (Join-Path $env:LOCALAPPDATA 'Polycode-Mainline')){throw 'Unexpected installation'};
        if(@(Get-ChildItem $env:TEMP -Filter 'polycode-mainline-download-*').Count){throw 'Download directory leaked'};
        'CALLER_ALIVE_FAILURE_HANDLED'`;
      const output = ok(run(runtime, script, directory));
      assert.match(output, /CALLER_ALIVE_FAILURE_HANDLED/);
      assert.ok(!output.includes('Polycode mainline installed.'));
  ...
  test(runtime + ': explicit production and Preview roots are rejected before download', () => {
    const script = `$script:downloads=0; function Invoke-WebRequest {$script:downloads++; throw 'Unexpected request'};
      ...
      $failed=$false; try { & ${quote(source)} -InstallRoot (Join-Path $env:LOCALAPPDATA 'Polycode-Preview-v0.2.1') } catch { $failed=$true };
      if(-not $failed -or $script:downloads -ne 0){throw 'Preview directory was not rejected early'}; 'ROOTS_UNCHANGED'`;
