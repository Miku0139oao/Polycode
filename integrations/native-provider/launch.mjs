...
export async function main(argv = process.argv.slice(2)) {
  ...
  return runNative({ binary: options.binary, cwd: options.cwd, provider: options.provider, resume: options.resume, nativeArgs, providers, directory: options['auth-directory'] ?? join(process.env.XDG_DATA_HOME || join(homedir(), '.local', 'share'), 'polycode', 'auth') });
}
