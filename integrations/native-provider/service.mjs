  ...
  async handle(req, res) {
    ...
    try {
      ...
      if (url.pathname === '/control/login/status' && req.method === 'GET') {
        ...
        return json(res, 200, { state: a.state, ...(a.message ? { message: a.message } : {}) });
      }
      if (url.pathname === '/control/login/cancel' && req.method === 'POST') {
        const b = await this.body(req), a = this.attempts.get(b.attemptId); if (!a) throw error('Unknown login attempt', 404);
        this.cancel(a); return json(res, 200, {});
      }
      const route = /^\/(codex|cursor)\/v1\/chat\/completions$/.exec(url.pathname);
      if (req.method === 'POST' && route) {
        const provider = route[1], body = await this.body(req);
      ...
      else json(res, e.status ?? 500, { error: { message: e.status ? e.message : 'Subscription provider operation failed. Check login, quota and provider availability.', type: 'polycode_provider_error' } });
