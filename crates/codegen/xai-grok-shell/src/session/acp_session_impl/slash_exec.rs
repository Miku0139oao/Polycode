...
impl SessionActor {
    ...
    pub(super) fn execute_builtin_slash_command(
        ...
        // Construct the dispatcher future on the heap, outside the input
        // future's storage. Keep every slash branch and its authority checks.
        Box::pin(async move {
            ...
            match action {
                ...
                BuiltinAction::SetYolo { enabled } => {
                    ...
                    if let Some(actual) = yolo_toggle_report(was, actual) {
                        self.emit_event(crate::session::events::Event::YoloToggled {
                            enabled: actual,
                        });
                ...
                BuiltinAction::SessionInfo => {
                    ...
                    let title = tokio::task::spawn_blocking(move || {
                        std::fs::read_to_string(&summary_path)
                            .ok()
                            .and_then(|raw| {
                                serde_json::from_str::<crate::session::persistence::Summary>(&raw)
                                    .ok()
                            })
                ...
                BuiltinAction::MemoryToggle { enabled } => {
                    ...
                    let msg = if enabled && !self.memory.is_enabled() {
                        ...
                        if !bridge.unregister_tool_by_name(
                            ...
                            tracing::debug!(
                                "memory_search tool was not registered during unregister"
                            );
                        }
                        if !bridge.unregister_tool_by_name(
