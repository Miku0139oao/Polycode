...
#[non_exhaustive]
pub enum AuthError {
    #[error("Not logged in. Run `polycode login`.")]
    NotLoggedIn,

    /// The token expired and no refresh authority is available.
    #[error("Token expired. Run `polycode login` to re-authenticate.")]
    TokenExpiredNoRefresh,

    /// Server rejected the token (401) with no recovery path.
    #[error("Authentication rejected by server. Run `polycode login` to re-authenticate.")]
    ...
    #[error("{message} Run `polycode login` to sign in with the required team.")]
    PinnedTeamMismatch { message: String },

    /// The cached API-key session was rejected because API-key auth is disabled.
    #[error("API-key auth is disabled by your administrator. Run `polycode login` to authenticate.")]
    ApiKeyAuthDisabled,
    ...
    pub(crate) fn user_message(self) -> Cow<'static, str> {
        match self {
            Self::RefreshTokenRejected => {
                "Your session has expired. Run `polycode login` to sign in again.".into()
            }
            Self::ClientRejected => {
                "Authentication is temporarily unavailable. Run `polycode login` if this persists."
                    .into()
            }
            Self::ProviderInteractiveRequired => provider_login_message(None),
            Self::Other => {
                "Authentication could not be refreshed. Run `polycode login` to sign in again.".into()
            }
        }
    }
}
