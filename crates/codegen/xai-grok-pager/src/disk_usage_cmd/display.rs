...
pub fn print_report(
    ...
    match report.registry {
        ...
        RegistryState::Corrupt => {
            writeln!(
                out,
                "  Worktree registry is damaged; rows show as untracked. Remove {} and run `polycode worktree db rebuild` to recreate it.",
                abbreviate(&report.registry_path, &report.grok_home, &home_label)
            )?;
        }
    }
    ...
    if report.worktrees_dominate() && !report.worktrees.is_empty() {
        ...
        if report.worktrees.iter().any(WorktreeUsage::is_tracked) {
            writeln!(
                out,
                "To reclaim space, run `polycode worktree gc --max-age 7d --dry-run`, then the same command without `--dry-run`. Without `--max-age`, gc expires nothing, and it keeps a worktree whose work it cannot find elsewhere, naming each one."
            )?;
        }
        if !report.worktrees.iter().all(WorktreeUsage::is_tracked) {
            writeln!(
                out,
                "Untracked rows are not in the registry, so gc never visits them. Remove one with `polycode worktree rm --dry-run <path>`, then without `--dry-run`."
            )?;
        }
    }
    Ok(())
