//! `polycode du`: what the user's Polycode home uses on disk.
//! It creates no Polycode home, registry file, or schema.
...
#[command(
    after_help = "Lists every top-level directory in the Polycode home, largest first, then every \
worktree under `worktrees/` and `worktree_pool/` with its size, age, and label. To reclaim space, preview a sweep with \
`polycode worktree gc --max-age 7d --dry-run`: without `--max-age`, gc expires nothing, it \
