...
/// The caller should print a message telling the user to relaunch `polycode`.
pub async fn run(
    mut args: PagerArgs,
    bg_update_rx: Option<
...
fn print_exit_resume_hint(info: &ExitInfo, max_width: usize, w: &mut impl Write) {
    ...
    if info.minimal {
        let _ = writeln!(w, "  polycode --minimal --resume {}", info.session_id);
    } else {
        let _ = writeln!(w, "  polycode --resume {}", info.session_id);
    }
}
...
mod tests {
    ...
    fn terminal_title_strips_control_characters() {
        assert_eq!(
            ...
            "evil]52;c;payloadtitle - Polycode"
        );
        assert_eq!(terminal_title_string("\x07\x1b\x00"), "Polycode");
        assert_eq!(terminal_title_string(""), "Polycode");
        assert_eq!(terminal_title_string("My chat"), "My chat - Polycode");
        assert_eq!(
            terminal_title_string(&"x".repeat(100)),
            format!("{} - Polycode", "x".repeat(69))
        );
    }
    #[test]
    ...
    #[test]
    fn cli_command_name_is_polycode() {
        use clap::CommandFactory;
        assert_eq!(PagerArgs::command().get_name(), "polycode");
    }
    #[test]
    fn cli_help_output_header() {
        ...
        assert_eq!(
            ...
            vec![
                ...
                "",
                "Usage: polycode [OPTIONS] [PROMPT] [COMMAND]",
                "",
                "Arguments:",
            ]
        );
    ...
    fn print_exit_resume_hint_includes_minimal_flag() {
        ...
        assert_eq!(
            String::from_utf8(buf).unwrap(),
            "\nResume this session with:\n  polycode --minimal --resume sess-abc\n"
        );
    }
    #[test]
    ...
    fn print_exit_resume_hint_truncates_summary_to_width() {
        ...
        assert!(out.contains(&format!("\n> {}…\n", "p".repeat(17))));
        assert!(out.contains(&format!("\n  {}…\n", "r".repeat(17))));
        assert!(out.contains("  polycode --resume sess-abc\n"));
    }
    #[test]
