...
fn mixed_report() -> DiagnosticReport {
    ...
    report.findings = vec![
        ...
        DiagnosticFinding {
            ...
            message: "Use local SSH wrapping".to_owned(),
            remediation: Some(ManualRemediation {
                fix: "polycode wrap ssh <host>".to_owned(),
                config_path: None,
            }),
            automatic_remediation: Some(crate::diagnostics::ssh_wrap_automatic_remediation()),
            note: None,
...
fn human_mixed_fixture_is_exact() {
    assert_eq!(
        ...
        concat!(
            ...
            "    → Automatic setup: `polycode doctor fix tmux-clipboard`\n",
            "    → Add `set -g set-clipboard on` to ~/.tmux.conf\n",
            "      Reload tmux after editing.\n",
            "  i terminal.ssh-wrap            Use local SSH wrapping\n",
            "    → Automatic setup: `polycode doctor fix ssh-wrap`\n",
            "    → One-off: `polycode wrap ssh <host>`\n",
            "\n",
            ...
            "  Some checks only run in Polycode. Start Polycode and run /doctor.\n",
            "\n",
...
fn fix_preview_contains_exact_change_and_caveats() {
    ...
    assert!(preview.contains("To use once without changing config: `polycode wrap ssh <host>`"));
...
fn json_contract_is_structural_stable_ordered_and_ansi_free() {
        ...
        serde_json::json!({
            ...
            "findings": [
                ...
                },
                {
                    ...
                    "remediation": {"fix": "polycode wrap ssh <host>", "configPath": null},
                    "automaticRemediation": {
                        "fixId": "terminal.ssh-wrap",
                        "command": "polycode doctor fix terminal.ssh-wrap"
                    },
                    "note": null
                }
            ],
    ...
    assert!(!text.contains("Polycode Doctor"));
}
...
fn new_named_findings_extend_json_without_schema_changes() {
    ...
    report.facts.clipboard.fix = Some("polycode wrap <ssh command> or /minimal".to_owned());
    report.findings.push(DiagnosticFinding {
    ...
    assert_eq!(
        ...
        "polycode wrap <ssh command> or /minimal"
    );
