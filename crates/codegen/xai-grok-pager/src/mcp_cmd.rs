//! `polycode mcp`: manage MCP server configurations from the command line.
...
Examples:
  ...
  polycode mcp add xcode -- xcrun mcpbridge

  # Add a stdio server with environment variables
  polycode mcp add postgres -e DATABASE_URL=postgres://localhost/mydb -- npx -y @modelcontextprotocol/server-postgres

  # Add a remote HTTP server
  polycode mcp add --transport http sentry https://mcp.sentry.dev/mcp

  # Add a remote server with an authentication header
  polycode mcp add --transport http api https://mcp.example.com/mcp --header \"Authorization: Bearer YOUR_TOKEN\"

  # Add to the project config (./.grok/config.toml) instead of ~/.grok/config.toml
  polycode mcp add --scope project github -- npx -y @modelcontextprotocol/server-github";
...
pub struct AddArgs {
    ...
    /// Place them after `--` so flags such as `-y` are passed to the server instead of Polycode.
    #[arg(value_name = "ARGS")]
    args: Vec<String>,
...
fn run_list(json: bool) -> Result<()> {
    ...
    if json {
        ...
        println!("No MCP servers configured. Run `polycode mcp add --help` to get started.");
    } else {
    ...
    match transport {
        McpTransport::Stdio => {
            let Some(command) = source else {
                bail!(
                    "A command is required for stdio servers. Usage: polycode mcp add <name> -- <command> [args...]"
                );
            };
        ...
        McpTransport::Http | McpTransport::Sse => {
            ...
            let Some(url) = source else {
                bail!(
                    "A URL is required for {label} servers. Usage: polycode mcp add --transport {label} <name> <url>"
                );
            };
...
async fn run_set_enabled(name: &str, enabled: bool) -> Result<()> {
    ...
    if !mcp_server_is_known(name, &cwd) {
        ...
        if !available.is_empty() {
            ...
            eprintln!("No MCP servers configured. Run `polycode mcp add --help` to get started.");
        }
        std::process::exit(1);
    }
...
async fn run_remove(name: &str, requested_scope: Option<McpScope>) -> Result<()> {
    ...
    let (scope, path) = match select_remove_site(user_defined, find_project_site(), requested_scope)
    {
        ...
        Err(RemoveError::Ambiguous { project_path }) => {
            ...
            eprintln!("Specify which one to remove, e.g.: polycode mcp remove {name} --scope project");
            std::process::exit(1);
        }
    };
