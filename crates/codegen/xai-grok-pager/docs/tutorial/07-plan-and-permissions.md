...
Polycode asks before doing anything risky — and can plan before it codes.

## Permissions

When the agent wants to run a risky command or edit a file, it pauses and asks:
...
For bigger or more ambiguous tasks, use **plan mode**: the agent explores the
...
`s` requests changes — the agent iterates until you're happy, then implements.
...
background — the agent keeps working and you're notified when it finishes
